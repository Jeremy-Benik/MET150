...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 16 14:59:26 2022
...
jan_ano = air_temp[0, :, :] - jan[:, :]
jul_ano = air_temp[6, :, :] - jul[:, :]
# %% Creating the figure
months = ["1950 January Temperature (\N{DEGREE SIGN}C) Anomaly (1950-1959)", "1950 July Temperature (\N{DEGREE SIGN}C) Anomaly (1950-1959)"]
vals = [jan_ano, jul_ano]
for i in range(len(vals)):
    ...
    cf = ax.contourf(lon, lat, vals[i][:, :], 200, cmap=plt.cm.RdBu_r, transform=ccrs.PlateCarree(), levels = np.linspace(-15, 15, 200))
...
jan_ano = air_temp[-13, :, :] - jan[:, :]
jul_ano = air_temp[-7, :, :] - jul[:, :]
# %% Creating the figure
months = ["1959 January Temperature (\N{DEGREE SIGN}C) Anomaly (1950-1959)", "1959 July Temperature (\N{DEGREE SIGN}C) Anomaly (1950-1959)"]
vals = [jan_ano, jul_ano]
for i in range(len(vals)):
    # Creating the plot
    fig = plt.figure(figsize = (12, 12)) #creating the figure and defining its size
