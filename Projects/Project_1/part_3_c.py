...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 16 15:19:40 2022
...
for i in time: #creating a for loop used to iterate through all the dates and times
    times.append(new_dt + datetime.timedelta(hours = i)) #appending the date onto the new list times
time = times[0:145]

# %%
time = np.array(time)
#Finding December 1957 for warmest temp
#print(np.where(time == datetime.datetime(1957, 12, 1, 0, 0))[0][0])
#finding November 1955 for coldest temp
#print(np.where(time == datetime.datetime(1955, 11, 1, 0, 0))[0][0])
# %% Calculating temp averages
#Calculating december mean
jan_temp = air_temp[0::12, :, :]
jan_temp = np.array(jan_temp)
jan = np.mean(jan_temp, axis = 0)
# temp average of that month for every year
nov_temp = air_temp[10::12, :, :]
nov_temp = np.array(nov_temp)
nov = np.mean(nov_temp, axis = 0)

# %% Temp anomaly
#Calculating the anomaly

dec_ano = air_temp[np.where(time == datetime.datetime(1958, 1, 1, 0, 0))[0][0], :, :] - jan[:, :]
nov_ano = air_temp[np.where(time == datetime.datetime(1955, 11, 1, 0, 0))[0][0], :, :] - nov[:, :]

# %% Creating the figure
months = ["January 1958 Surface Temperature Anomaly (\N{DEGREE SIGN}C) 1950-1959", "November 1955 Surface Temperature Anomaly (\N{DEGREE SIGN}C) 1950-1959"]
vals = [dec_ano, nov_ano]
for i in range(len(vals)):
    ...
    cf = ax.contourf(lon, lat, vals[i][:, :], 500, cmap=plt.cm.RdBu_r, transform=ccrs.PlateCarree(), levels = np.linspace(-14, 14, 500))
